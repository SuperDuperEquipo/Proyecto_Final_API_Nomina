...
describe('ConfiguracionVigenteService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(ConfiguracionDeduccion), useValue: mockConfigRepository },
        { provide: getRepositoryToken(TramoISR), useValue: mockTramoRepository },
      ],
    }).compile();

    service = module.get<ConfiguracionVigenteService>(ConfiguracionVigenteService);
  ...
  describe('obtenerConfiguracionVigente', () => {
    it('retorna la configuración cuando el repositorio la encuentra', async () => {
      const config = { id: 1, vigenteDesde: new Date('2025-05-08') } as ConfiguracionDeduccion;
      configRepository.findOne.mockResolvedValue(config);

      const resultado = await service.obtenerConfiguracionVigente(new Date('2026-07-20'));
    ...
    it('lanza UnprocessableEntityException si no hay configuración para esa fecha', async () => {
      ...
      await expect(service.obtenerConfiguracionVigente(new Date('2020-01-01'))).rejects.toThrow(
  ...
  describe('obtenerTramosIsrVigentes', () => {
    it('retorna los tramos cuando el repositorio los encuentra', async () => {
      ...
      const resultado = await service.obtenerTramosIsrVigentes(new Date('2026-07-20'));
    ...
    it('lanza UnprocessableEntityException si no hay tramos para esa fecha', async () => {
      ...
      await expect(service.obtenerTramosIsrVigentes(new Date('2020-01-01'))).rejects.toThrow(
        UnprocessableEntityException,
      );
    });
  });
