...
3. **`afectaBasePrestaciones` para distinguir salario de liberalidad:** una `BONIFICACION` habitual y recurrente es salario y debe entrar en la base de cálculo de aguinaldo y vacaciones, una liberalidad ocasional del patrono. El campo lo decide quien registra la novedad, y el servicio lo respeta tal cual salvo para `LICENCIA_MATERNIDAD`.
4. **No se creó un tipo `COMISION` separado:** no es una de las funcionalidades explícitamente requeridas en el alcance original del proyecto (solo horas extra, permisos sin goce, bonificaciones y descuentos). Si en el futuro se necesita, se recomienda modelarla como `BONIFICACION` usando el campo `fecha` para representar la fecha en que el cliente pagó no la fecha de la venta y vita sobre-diseñar el módulo para un caso no pedido.
...
- **Depende de:**
  ...
  - **Nómina (P4):** relación `nominaId` contra la entidad `Nomina`. Como el módulo de P4 todavía no existe, se creó una entidad `Nomina` mínima (`src/nomina/entities/nomina.entity.ts`) con `id`, `periodo`, `tipo` y `estado`, como contrato compartido — P4 debe expandirla, no reemplazarla, para no romper esta relación.
- **A quién le sirve:**
  - **Nómina (P4):** consumirá `GET /novedades/nomina/:nominaId` para sumar horas extra, bonificaciones y descuentos al cerrar la planilla del período.
  - **Comprobantes (P5):** indirectamente, a través del cálculo que haga P4 con estas novedades.
